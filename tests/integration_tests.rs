...
const TEST_DB_BASE_PATH: &str = "./.lancedb-test";
...
async fn setup_test_env(test_name: &str) -> (Storage, Embedder, CodeChunker, String) {
...
fn prepare_chunks(chunks: &[code_rag::indexer::CodeChunk]) -> (Vec<String>, Vec<String>, Vec<String>, Vec<i32>, Vec<i32>, Vec<i64>, Vec<Vec<String>>) {
    ...
    (ids, filenames, codes, line_starts, line_ends, last_modified, calls)
}
...
async fn test_index_test_assets() {
    ...
    let test_files = vec![
        "test.rs", "test.py", "test.go", "test.js", "test.java",
        "test.css", "test.html", "test.json", "test.yaml", 
        "test.sh", "test.ps1"
    ];
    ...
    assert!(total_chunks > 20, "Expected at least 20 chunks, got {}", total_chunks);
...
async fn test_multi_language_search() {
    ...
    println!("✓ Multi-language search found {} results from {} chunks", results.len(), total_chunks);
...
fn test_language_detection() {
    ...
    assert!(CodeChunker::get_language("rs").is_some(), "Rust not detected");
    assert!(CodeChunker::get_language("py").is_some(), "Python not detected");
    assert!(CodeChunker::get_language("go").is_some(), "Go not detected");
    assert!(CodeChunker::get_language("js").is_some(), "JavaScript not detected");
    assert!(CodeChunker::get_language("sh").is_some(), "Bash not detected");
    assert!(CodeChunker::get_language("ps1").is_some(), "PowerShell not detected");
    assert!(CodeChunker::get_language("json").is_some(), "JSON not detected");
    assert!(CodeChunker::get_language("yaml").is_some(), "YAML not detected");
    assert!(CodeChunker::get_language("unknown").is_none(), "Unknown extension should return None");
...
"#;
    ...
    assert!(chunks.len() >= 2, "Expected at least 2 chunks (main + helper), got {}", chunks.len());
    assert!(chunks.iter().any(|c| c.code.contains("main")), "Should contain main function");
    assert!(chunks.iter().any(|c| c.code.contains("helper")), "Should contain helper function");
...
"#;
    ...
    assert!(chunks.iter().any(|c| c.code.contains("greet") || c.code.contains("Calculator")), 
