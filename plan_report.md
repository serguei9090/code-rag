...
### 2. Progress Indicators for Model Loading
...
### 3. Path Normalization During Indexing
...
### 4. Model Pre-caching During Index
...
### 6. LanceDB Index on `filename` Column
...
### 7. Persistent Server Mode (`code-rag serve`)
- **Estimated LOC**: ~350 lines
- **Files**: New `server.rs`, `main.rs`
- **Impact**: All searches become instant (~0.5-1s) after initial startup
- **Complexity**: High
- **Description**: HTTP/gRPC server that keeps models in memory. Clients query via API.
...
### 11. Query Expansion with Local LLM
...
|---|---------|-----|------------|-------------------|------------|
| 1 | `--no-rerank` flag | 15 | ⭐ | ⚡⚡⚡ (56s → 1s) | High |
| 2 | Progress indicators | 25 | ⭐ | 📊 (UX only) | Medium |
| 3 | Path normalization | 10 | ⭐ | ⚡ (2-3s savings) | Low |
| 4 | Model pre-caching | 80 | ⭐⭐ | ⚡⚡⚡ (56s → 2s) | High |
| 5 | Batch size increase | 30 | ⭐ | ⚡⚡ (2-3x faster indexing) | Medium |
