...
function Show-Menu {
    ...
    Write-Host "4. Build All (CPU)"
    Write-Host "5. Build Windows (CUDA)"
    Write-Host "6. Build Linux (CUDA - Docker)"
    Write-Host "Q. Quit"
    Write-Host "===========================" -ForegroundColor Cyan
}
...
do {
    ...
    switch ($choice) {
        ...
        "5" {
            Write-Host "Starting Windows Build (CUDA)..." -ForegroundColor Yellow
            & "$PSScriptRoot\scripts\build-windows-cuda.ps1"
            Pause
        }
        "6" {
            Write-Host "Starting Linux Build (CUDA)..." -ForegroundColor Yellow
